...
- [ ] `get_top_market_cap_data` 라인 246: `entry["market_cap"] = int(round(mcap * _STCK_AVLS_UNIT_TO_WON))` 변경 (round 적용으로 부동소수점 절단 보호)
- [ ] mcap > 0 가드 유지 (음수/0/NaN 종목 entry["market_cap"] 미생성)
- [ ] 인라인 주석 갱신 (라인 239-244): "단위 미확정" → "단위 = 억원 (단위 2-9e 확정), × 1억 정규화 적용"
- [ ] 진단 로그 갱신 (라인 247-253): "원 단위 가정. min_market_cap=5,000억" 오기 → "단위 2-9e 확정: 억원 × 1억 정규화. min_market_cap=500억과 비교"
- [ ] py_compile 통과

### Step 2 — `universe_filters.py` DEFAULT 값 정합 (안전망 보강)
- [ ] 라인 86: `DEFAULT_FALLBACK_INCLUDE_MARKET_CAP = True` → `False`
- [ ] 변경 사유 주석 추가 (settings 로드 실패 시 안전망 정합)
- [ ] py_compile 통과

### Step 3 — `settings.yaml` 토글 활성화
- [ ] `data_source.fallback_include_market_cap: false` → `true` 변경
- [ ] 주석에 단위 2-9e 활성 배경 + 사전 조사 5종목 단위 확정 결과 추가
- [ ] yaml 문법 검증
...
- [ ] MC-6a: 정규화 적용 → market_cap=60,000,000,000 (600억) → `min_market_cap=50_000_000_000` (500억) 임계 통과 PASS
- [ ] MC-6b (회귀): 정규화 누락 시뮬 → market_cap=600 → 500억 미달 FAIL (회귀 검출)
...
- [ ] code-tester 에이전트 호출 (수정 파일 3개 + 테스트 1개 대상)
...
### 단위 검증
- [ ] py_compile 통과 (kis_market_provider.py + universe_filters.py + scripts/test_closing_bet_unit_2_9e.py)
- [ ] `venv/bin/python scripts/test_closing_bet_unit_2_9e.py` 11건 PASS
- [ ] 회귀 91건: 2-9a 18 + 2-9b 15 + 2-9c 10 + 2-9d 12 + 2-9e 11 + 2-2b-1 15 + 2-2b-2 10 = **91건 PASS**

### 통합 검증 (단발)
- [ ] `get_universe_v2_filtered()` 단발 호출
- [ ] KIS market-cap 호출 로그 1건 + 보강 종목 수 ≥ 30건 (상위 200 ∩ universe 63 추정)
- [ ] `[kis_market_provider] stck_avls 단위 = 억원 (단위 2-9e 확정), × 1억 정규화 적용` 로그 1회
- [ ] universe ≥ 1건 (단발 검증, 5건은 자연 트리거 의존)
- [ ] KIS API 호출 시간 ≤ 6초 (4 ranking + 1 market-cap)

### 실전 검증 (5/7 또는 다음 영업일 자연 트리거)
- [ ] 15:10 KST 자동 트리거 발화
- [ ] `[universe_v2] 출처별 기여` 모두 비-0
- [ ] `[universe_filters] 시총 보강` 로그 — 매치 종목 수 ≥ 30 + 미매치 탈락 카운트
- [ ] universe ≥ 5건 (목표 달성)
- [ ] candidates INSERT ≥ 1건
- [ ] 일일 요약 텔레그램 알림 (15:35)
- [ ] **신규**: pykrx bulk 정상 복구 여부 점검 (시총 보강 로그 미발생 시 bulk 복구 의심)
- [ ] **신규**: universe 에 ETF(069500 등)/우선주(005935 등) 0건 확인 (단위 2-9f 트리거 점검)
...
## 문서 업데이트 항목
- [ ] `docs/improvements/change_log.md` 1줄 추가 (단위 2-9e 항목, 단위 2-9d 형식 일관)
- [ ] `memory/project_closing_bet_system.md` 단위 2-9e 1단락 추가 (시총 보강 정규화)
- [ ] `memory/MEMORY.md` 인덱스 description 갱신 (closing_bet_system 항목)
- [ ] git commit + push
- [ ] 임시 스크립트 `scripts/probe_kis_unit_2_9e.py` **보존** (사용자 결정 확정 — 재검증용)
...
- [ ] 자연 트리거 검증 후 → active → completed 아카이브 (단위 2-9c/2-9d 와 함께)
